...
describe('GET /api/tasks filters', () => {
  ...
  const createTaskAt = (deadline: Date, owner = teacher) =>
    prisma.task.create({
      data: { title: 'Filtered', description: 'filter test', deadline, createdBy: owner.id },
    });

  const ids = (res: request.Response) => res.body.tasks.map((t: { id: number }) => t.id);

  it('mine=true lists only tasks the requester created', async () => {
    const own = await createTaskAt(futureDate(), otherTeacher);
    const notOwn = await createTaskAt(futureDate(), teacher);
    const res = await request(app).get('/api/tasks?mine=true&limit=100').set(auth(otherTeacher.token));
    expect(res.status).toBe(200);
    expect(ids(res)).toContain(Number(own.id));
    expect(ids(res)).not.toContain(Number(notOwn.id));
    expect(res.body.tasks.every((t: { createdBy: number }) => t.createdBy === Number(otherTeacher.id))).toBe(true);
    expect(res.body.total).toBe(res.body.tasks.length);
  });

  it('status=open keeps upcoming deadlines only, soonest first', async () => {
    const closed = await createTaskAt(pastDate());
    const later = await createTaskAt(futureDate(20));
    const sooner = await createTaskAt(futureDate(10));
    const res = await request(app).get('/api/tasks?status=open&mine=true&limit=100').set(auth(teacher.token));
    expect(res.status).toBe(200);
    expect(ids(res)).not.toContain(Number(closed.id));
    expect(ids(res).indexOf(Number(sooner.id))).toBeLessThan(ids(res).indexOf(Number(later.id)));
  });

  it('status=closed keeps passed deadlines only, most recent first', async () => {
    const open = await createTaskAt(futureDate());
    const older = await createTaskAt(pastDate(5));
    const recent = await createTaskAt(pastDate(1));
    const res = await request(app).get('/api/tasks?status=closed&mine=true&limit=100').set(auth(teacher.token));
    expect(res.status).toBe(200);
    expect(ids(res)).not.toContain(Number(open.id));
    expect(ids(res).indexOf(Number(recent.id))).toBeLessThan(ids(res).indexOf(Number(older.id)));
  });

  it('rejects an unknown status or mine value with 400', async () => {
    for (const query of ['status=late', 'mine=yes']) {
      const res = await request(app).get(`/api/tasks?${query}`).set(auth(teacher.token));
      expect(res.status).toBe(400);
    }
  });
});
