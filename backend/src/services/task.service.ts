...
export const taskService = {
  ...
  async list({ page, limit, status, mine }: ListTasksQuery, requesterId: bigint) {
    const [tasks, total] = await taskRepository.findPage((page - 1) * limit, limit, {
      status,
      ...(mine && { createdBy: requesterId }),
    });
    return {
  ...
  async listSubmissions(taskId: bigint, teacherId: bigint) {
    ...
    return {
      ...
      submissions: submissions.map(({ user, ...submission }) => ({
        ...toSubmissionDto(submission),
        student: { id: Number(user.id), username: user.username, email: user.email },
      })),
    };
  },
